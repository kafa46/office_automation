...
class DocumentFactory(ABC):
    ...
    @abstractmethod
    def read_doc(self,file_name: str) -> object:
...
class CMSCDocumentFactory(DocumentFactory):
    ...
    def read_doc(self, file_name: str) -> object:
        ...
        # 나도 잘 모름... -> 요기 참고함, https://wikidocs.net/72208
        import pythoncom
    ...
    def gen_doc(self, hwp: object, doc_info: dict) -> dict:
        try: 
                ...
                if field_name in doc_info.keys():
                    ...
                    if field_data: # 입력할 데이터가 있으면 적절히 입력
                        if  field_name.endswith('sign'): # 이미지 입력할 경우
                            ...
                            print(f'field_name: {field_name}, field_data: {field_data}')
            ...
            adjust_page_ending(hwp) # 페이지 마무리

            # 저장 및 종료
            hwp.MovePos(2) # 문서 시작으로 이동
            hwp.Save() # 저장
            hwp_path =  hwp.XHwpDocuments.Item(0).FullName # 파일 경로 추출
            pdf_path = save_as_pdf(hwp, hwp_path) # pdf로 추가 저장
            hwp.Quit() #  메모리 반환 후 한글 종료
            success = True
...
def builder(factory: DocumentFactory, doc_info: dict) -> dict:
    '''Main function.
    Return: dict = {
        'object': hwp object,
        'hwp_path': save_path: str,
        'pdf_path': save_path: str,
        'status': True|False: bool,
    }'''
    doc_info = factory.get_data(doc_info)
    hwp = factory.read_doc(doc_info['file_name'])
    hwp.XHwpWindows.Item(0).Visible = False
    return factory.gen_doc(hwp, doc_info)